...
<form action="{{ route('events.update', $event->id) }}" method="POST" class="bg-white p-8 rounded-lg max-w-lg mx-auto">
    ...
    <div class="mb-6 @if($event->selected_game != 'singles') hidden @endif" id='single'>
    ...
    <div class="mb-4 @if($event->selected_game != 'doubles') hidden @endif" id='double'>
    ...
    <div class="mb-4">
        ...
        <select id='game_type' class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" name='selected_game'>
            ...
            <option value="singles" {{ $event->selected_game == 'singles' ? 'selected' : '' }}>Singles</option>
            <option value="doubles" {{ $event->selected_game == 'doubles' ? 'selected' : '' }}>Doubles</option>
        </select>
    </div>
